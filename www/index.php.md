...
COMPOSER_ROOT       指明Composer的vendor目录的文件路径。
APP_ROOT            指明App目录的文件路径。
                        App目录是项目目录，保存App项目的相关程序和配置。
VAR_ROOT            指明Var目录的文件路径。
                        Var目录是用于存储动态生成文件的目录（如cache文件，log文件等），此目录要有可写权限。
WEB_ROOT            指明Web目录的文件路径。
                        一般就是入口文件的所在目录。
...
DIDA_DEBUG_MODE     指明是否开启调试模式，值为true或者false。默认是开启true。
                        开启后，会显示所有错误信息。
WEB_BASE            指明web的根路径，默认是入口文件所在的web路径。
                        一般这个值为/，虚拟目录则可能为/foo/，最后一个字符应为/。
                        程序在路由时，会忽略网址中 WEB_BASE 对应的开头部分，以获取一个和主机无关的web路径。
SANDBOX_ROOT        指明沙箱目录的文件路径。默认和VAR_ROOT一致。
                        沙箱目录用于隔离安全性不明的文件，如客户上传的文件等。
                        这个目录要有可写权限。
```
